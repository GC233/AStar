...
class CellField:
    #需要一个工厂函数
    def __init__(self, cls, row:int=1, col:int=1):
        #生成一个row行col列，值为None的二维数组
        self.__field=[[cls() for i in range(col)] for i in range(row)]
    ...
    def getNeighbour(self, row:int, col:int):
        "返回p[row][col]周围<=8个邻居"
        if not self.__field:
            return None
        result=[]
        ...
        for i in range(startRow, startRow+3):
            for j in range(startCol, startCol+3):
                #跳过自己和越界访问
                if (i==row and j==col) or i>=len(self.__field) or i<0 or j>=len(self.__field[0]):
                    continue
                else:
